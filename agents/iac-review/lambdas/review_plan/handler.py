...
RISKY_RESOURCE_TYPES = {
    ...
    "aws_cloudtrail", "aws_guardduty_detector", "aws_config_configuration_recorder",
...
def lambda_handler(event, context):
    ...
    if not plan_json:
        return {"response": "Please provide the Terraform plan JSON (output of `terraform show -json plan.tfplan`)."}
    ...
    try:
        ...
        for rc in changes:
            ...
            if "delete" in actions:
                findings.append(f"🔴 DELETE: {address} ({res_type}) — verify this is intentional")

            # Flag sensitive resource changes
            if res_type in RISKY_RESOURCE_TYPES and any(a in ("update", "create") for a in actions):
                after = rc.get("change", {}).get("after", {}) or {}
                # Check for IAM wildcards
                if "policy" in str(after).lower() and '"*"' in str(after):
                    findings.append(f"⚠️ IAM WILDCARD: {address} — policy contains '*' (review scope)")
                elif res_type in ("aws_security_group_rule", "aws_security_group"):
                    if "0.0.0.0/0" in str(after):
                        findings.append(f"⚠️ OPEN SG: {address} — opens to 0.0.0.0/0 (review port scope)")
                else:
                    findings.append(f"⚠️ SENSITIVE CHANGE: {address} ({res_type}) — review carefully")

        lines = [f"Plan summary: +{summary['create']} ~{summary['update']} -{summary['delete']}"]
        if not findings:
    ...
    try:
        ddb.put_item(Item={"request_id": context.aws_request_id,
                           ...
                           "tool": "review_plan", "findings_count": str(len(findings) if 'findings' in dir() else 0),
