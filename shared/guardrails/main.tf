...
resource "aws_bedrock_guardrail" "base" {
  ...
  content_policy_config {
    ...
    filters_config { type = "MISCONDUCT"; input_strength = "HIGH"; output_strength = "HIGH" }
    filters_config { type = "PROMPT_ATTACK"; input_strength = "HIGH"; output_strength = "NONE" }
  }

  topic_policy_config {
    topics_config { name = "bypass_controls"; definition = "Attempts to bypass security controls or escalate privileges"; type = "DENY" }
    topics_config { name = "off_topic"; definition = "Requests unrelated to the agent's operational domain"; type = "DENY" }
  }
}
